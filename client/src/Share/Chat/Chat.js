...
function Chat(props) {
	...
	const handlerSend = async () => {
		...
		if (!roomId) {
			...
			setRoomId(newRoomData.idRoom);
			localStorage.setItem('njs_asm3_roomId', newRoomData.idRoom);
			setTimeout(() => {
				socket.emit('createRoom', 'create a room');
			}, 100);
		}

		const data = {
			message: textMessage,
			roomId: roomId || localStorage.getItem('njs_asm3_roomId'),
			is_admin: false,
		};
		setTimeout(() => {
						...
						<div className='card card-bordered fix_boderChat' style={{ width: "fit-content" }}>
							...
							<div className='ps-container ps-theme-default ps-active-y fix_scoll'>
								{message && message.length > 0 &&
									message.map((value, i) => !value.is_admin ? (
										<div
