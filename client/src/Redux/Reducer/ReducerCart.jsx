	...
	switch (action.type) {
		...
		case 'ADD_CART':
			...
			if (add_cart.length < 1) {
				...
				const indexCart = add_cart.findIndex((value) => value.idProduct === data_add_cart.idProduct);

				//Tìm xem thử sản phẩm này đã mua hay chưa
				const findCart = add_cart.find((value) => value.idProduct === data_add_cart.idProduct);
